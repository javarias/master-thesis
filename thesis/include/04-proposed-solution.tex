...
This problem depends upon the result of the algorithm output for the ``Scheduling Array Problem'' as illustrated on figure~\ref{fig:array-config-problem-overview}. This problem is part of the ``long-term scheduling'' problem only, therefore multiple Array configurations are considered as input.
\begin{figure}[h!]
\begin{center}
...
The antenna movement duration is not considered in the model.
...
The main objective of the algorithm for this problem, is the same defined for the ``ALMA Scheduling Array Problem'' in section~\ref{sec:array-sched-math-model} equations~\ref{eq:max-sci-value} and~\ref{eq:min-idle-time}. To maximize the scientific value of the completed projects and to minimize the instrument's idle time, respectively.

\subsubsection{Constraints}
\begin{description}
\item[Array configurations duration overlap] \hfill \\
The antennas and the correlators computers are unique instrument components that can be present in just one array configuration at the very same time. Hence, to avoid duplications of resources, just one type of array configuration of the same type is allowed at a given time period. There is one exception, the Total power arrays, but for simplification it is considered as the other configuration types. The constraint can be expressed as shown by equation~\ref{eq:array-uniqness}
