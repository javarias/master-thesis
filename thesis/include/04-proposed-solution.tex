...
The search space of the problem is related to the combinations that each array configuration's start date and duration can produce, modifying either its start time, duration or both, during the span of the whole observing season. Considering a whole year ($52\,[weeks]$) and just one array configuration there are $\Sigma_{i=4}^{52} (52 - i) = 1128$ different starting dates and duration for the array configuration. Adding more array configurations make the number of possibilities to explode exponentially.
...
Although the effectiveness of the method is not guaranteed, given the scheduling blocks' sky source dispersion over the sky, in particular over the Right Ascension coordinate.

\begin{algorithm}                     
\caption{Simulated Annealing}          
\label{alg:array-sa}                   
\begin{algorithmic}                    
    \REQUIRE $ToBeWritten$ and $i = 0$
\end{algorithmic}
\end{algorithm}
